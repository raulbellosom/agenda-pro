...
import { must, safeBodyJson, json, getVerificationEmailHtml, sendEmailWithNodemailer } from "./_shared.js";
...
function logRuntimeInfo(log) {
  log(`Runtime: node=${process.version}, platform=${process.platform}, arch=${process.arch}`);
...
async function listDocuments({ log, endpoint, projectId, apiKey, dbId, collId, queries }) {
...
async function createDocument({ log, endpoint, projectId, apiKey, dbId, collId, data }) {
  const url = buildAppwriteUrl(endpoint, `/databases/${dbId}/collections/${collId}/documents`);
  const r = await appwriteFetch({
    log,
    method: "POST",
    url,
...
async function updateDocument({ log, endpoint, projectId, apiKey, dbId, collId, docId, data }) {
  const url = buildAppwriteUrl(
    endpoint,
...
function getAppwriteEndpoint(log) {
  ...
  let endpoint = process.env.APPWRITE_FUNCTION_API_ENDPOINT || process.env.APPWRITE_ENDPOINT;
...
export default async ({ req, res, log, error }) => {
  ...
  if (DEBUG) {
    ...
    log(`Config: db=${process.env.APPWRITE_DATABASE_ID}, profiles=${process.env.PROFILES_COLLECTION_ID}, verifications=${process.env.EMAIL_VERIFICATIONS_COLLECTION_ID}`);
  }

  const DB_ID = must("APPWRITE_DATABASE_ID");
  const PROFILES_COLL_ID = must("PROFILES_COLLECTION_ID");
  const VERIFICATIONS_COLL_ID = must("EMAIL_VERIFICATIONS_COLLECTION_ID");
  const FRONTEND_URL = must("FRONTEND_URL");

  try {
    const body = safeBodyJson(req);
    
    // Obtener action del body o query string
    const action = body.action || new URL(req.url, "http://localhost").searchParams.get("action") || "send";
    ...
    if (action === "send") {
      ...
      const emailHtml = getVerificationEmailHtml(token, FRONTEND_URL);

      // Enviar email con Nodemailer
      log(`Attempting to send email to: ${email}`);
      log(`SMTP Configuration: host=${process.env.SMTP_HOST}, port=${process.env.SMTP_PORT}, secure=${process.env.SMTP_SECURE}`);
      
      try {
        await sendEmailWithNodemailer(email, "Verifica tu email - Agenda Pro", emailHtml);
    ...
    if (action === "verify") {
      ...
      const profileDocs2 = await listDocuments({
        ...
        collId: PROFILES_COLL_ID,
        queries: [
          { method: "equal", attribute: "userAuthId", values: [tokenDoc.userAuthId] },
          { method: "limit", values: [1] },
        ],
      });
    ...
    if (action === "resend") {
      ...
      const oldTokens = await listDocuments({
        ...
        collId: VERIFICATIONS_COLL_ID,
        queries: [
          { method: "equal", attribute: "userAuthId", values: [profile.userAuthId] },
          { method: "equal", attribute: "verified", values: [false] },
        ],
      });
